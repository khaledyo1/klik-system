...
const SECTION_META = {
  ...
  sales: ['المبيعات', 'بحث، فلترة، تعديل، وتجديد العمليات'],
  analytics: ['التقارير', 'تحليل الربحية والمخاطر والمصاريف'],
  customers: ['العملاء', 'ملفات العملاء والتجديدات والتواصل'],
  expenses: ['المصاريف', 'تسجيل ومتابعة مصاريف التشغيل'],
  ...
  settings: ['توزيع الأرباح', 'إدارة نسب الأرباح والطوارئ ورأس المال'],
...
function canManage() {
  return ['SuperAdmin', 'Manager', 'آدمن', 'ادمن', 'مدير عام'].includes(myProfile.role) || isSuperAdmin();
}
...
function isSuperAdmin() {
  ...
  return role === 'SuperAdmin' || role === 'آدمن' || role === 'ادمن' || role === 'مدير عام' || email === adminEmail;
}
...
function stopRealtime() {
  ...
  dbUnsubscribe = usersUnsubscribe = auditUnsubscribe = expensesUnsubscribe = null;
}
...
function openModalForAdd() {
  ...
  if (byId('inPurchaseSource')) byId('inPurchaseSource').value = '';
  if (byId('inFollowupStatus')) byId('inFollowupStatus').value = 'جديد';
  if (byId('inFollowupNote')) byId('inFollowupNote').value = '';
  if (byId('duplicateCustomerHint')) byId('duplicateCustomerHint').classList.add('hidden');
...
function openModalForEdit(id) {
  ...
  byId('inSupplier').value = sale.customerPaymentMethod || sale.supplier || '';
  if (byId('inPurchaseSource')) byId('inPurchaseSource').value = sale.purchaseSource || '';
  if (byId('inFollowupStatus')) byId('inFollowupStatus').value = sale.followupStatus || 'جديد';
  if (byId('inFollowupNote')) byId('inFollowupNote').value = sale.followupNote || '';
...
byId('saleForm').addEventListener('submit', async (e) => {
  ...
  const data = {
    ...
    customerPaymentMethod: byId('inSupplier').value.trim(),
    purchaseSource: byId('inPurchaseSource')?.value.trim() || '',
    followupStatus: byId('inFollowupStatus')?.value || 'جديد',
    followupNote: byId('inFollowupNote')?.value.trim() || '',
    renewedFromSaleId: byId('editSaleId')?.dataset?.renewedFromSaleId || '',
...
function populateDash() {
  ...
  renderLiquidityDashboard(filtered);
  renderMobileSalesCards();
}
...
function currentSalesFilter() {
  ...
  return periodFiltered.filter((s) => {
    const text = `${s.service || ''} ${s.linkedSaleService || ''} ${s.clientName || ''} ${s.linkedSaleClientName || ''} ${s.clientPhone || ''} ${s.phoneNormalized || ''} ${s.customerPaymentMethod || s.supplier || ''} ${s.purchaseSource || ''} ${s.label || ''} ${s.accountStatus || ''} ${s.followupStatus || ''} ${s.compensationReason || ''}`.toLowerCase();
      ...
      <tr class="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
        <td class="px-6 py-4">
          ...
          <div class="mt-2 flex flex-wrap gap-1">
            ...
            ${(s.customerPaymentMethod || s.supplier) ? `<span class="badge badge-slate">دفع: ${escapeHtml(s.customerPaymentMethod || s.supplier)}</span>` : ''}
            ${s.purchaseSource ? `<span class="badge badge-slate">مصدر: ${escapeHtml(s.purchaseSource)}</span>` : ''}
            ${s.followupStatus ? `<span class="badge badge-sky">متابعة: ${escapeHtml(s.followupStatus)}</span>` : ''}
            <span class="badge ${acct === 'تم التعويض' || acct === 'مشكلة في الحساب' ? 'badge-red' : acct === 'قيد المتابعة' || acct === 'بانتظار الدفع' || acct === 'بانتظار التفعيل' ? 'badge-orange' : 'badge-green'}">${escapeHtml(acct)}</span>
          </div>
        </td>
        ...
        <td class="px-4 py-4 text-center">
          <div class="flex items-center justify-center gap-2 flex-wrap">
            ...
            <button onclick="openRenewalFromSale('${s.id}')" class="icon-btn text-sm" title="تجديد">🔁</button>
            <button onclick="quickFollowupUpdate('${s.id}')" class="icon-btn text-sm" title="تحديث المتابعة">📌</button>
    ...
    <tr class="bg-orange-50/70 dark:bg-orange-950/10 hover:bg-orange-50 dark:hover:bg-orange-950/20 transition-colors">
      <td class="px-6 py-4">
        ...
        <div class="mt-2 flex flex-wrap gap-1">
          ...
          ${(s.customerPaymentMethod || s.supplier) ? `<span class="badge badge-slate">وسيلة الدفع: ${escapeHtml(s.customerPaymentMethod || s.supplier)}</span>` : ''}
...
function renderPaymentMethodList() {
  const items = aggregateSalesBy((s) => s.customerPaymentMethod || s.supplier || 'غير محدد').sort((a, b) => b[1].revenue - a[1].revenue).slice(0, 10);
...
function exportAnalyticsReport() {
  ...
  aggregateSalesBy((s) => s.customerPaymentMethod || s.supplier || 'غير محدد').forEach(([label, v]) => rows.push(['وسائل الدفع', label, v.count, n(v.revenue).toFixed(3), n(v.profit).toFixed(3), n(v.compensation).toFixed(3)]));
...
async function emptyTrash() {
  if (!isSuperAdmin()) return toast('تفريغ السلة متاح للآدمن فقط.', 'error');
...
function actionLabel(action) {
  const labels = {
    ...
    sale_renewed: 'تجديد اشتراك',
    followup_updated: 'تحديث متابعة'
  };
...
const BACKUP_COLLECTIONS = ['sales', 'users', 'auditLogs', 'invites', 'expenses'];
...
async function restoreFullBackup() {
  ...
  if (replaceMode) {
    const typed = (byId('backupConfirmInput')?.value || '').trim();
    if (typed !== 'استعادة') return toast('للاستبدال الكامل اكتب كلمة استعادة في مربع التأكيد.', 'warning');
  }
...
function exportData(period) {
  ...
  const headers = ['نوع الحركة', 'التاريخ', 'الخدمة', 'العميل', 'الهاتف', 'وسيلة دفع الزبون', 'مصدر الشراء', 'عملة الشراء', 'حالة الحساب', 'الإيراد', 'تكلفة الشراء', 'تعويض فوري', 'تعويض من الطوارئ', 'الربح الحقيقي', 'رأس مال إضافي', 'رصيد/أثر الطوارئ', 'صافي أرباح', 'المسؤول', 'تاريخ الانتهاء'];
  const csvRows = [headers.join(',')];
  filtered.forEach((s) => {
    const row = [
      isCompensationRecord(s) ? 'تعويض من الطوارئ' : 'مبيعة', transactionDate(s), s.service || s.linkedSaleService || '', s.clientName || s.linkedSaleClientName || '', s.clientPhone || s.linkedSaleClientPhone || '', s.customerPaymentMethod || s.supplier || '', s.purchaseSource || '', s.purchaseCurrency || 'USD', s.accountStatus || (isCompensationRecord(s) ? 'تعويض من الطوارئ' : 'فعال'),
...
// ==========================================
// Enhancements V8 - Customers, Renewal, Expenses, Follow-up, Liquidity, PWA
// ==========================================

function paymentMethodOf(item) {
  return item?.customerPaymentMethod || item?.supplier || item?.paymentMethod || 'غير محدد';
}

function purchaseSourceOf(item) {
  return item?.purchaseSource || item?.sourceName || '';
}

function startExpensesSync() {
  if (expensesUnsubscribe) expensesUnsubscribe();
  expensesUnsubscribe = db.collection('expenses')
    ...
    .onSnapshot((snap) => {
      ...
      console.warn('Expenses sync failed', err);
      expenses = [];
      renderExpenses();
    });
}

function expensesTotal(list = expenses) {
  return list.filter((x) => !x.isDeleted).reduce((sum, x) => sum + n(x.amount), 0);
}

function expensesByMethod(list = expenses) {
  const map = new Map();
  list.filter((x) => !x.isDeleted).forEach((x) => {
    const key = paymentMethodOf(x);
    map.set(key, (map.get(key) || 0) + n(x.amount));
  });
  return map;
}

function renderLiquidityDashboard(filteredSales = getFilteredForDashboard ? getFilteredForDashboard() : liveRecords(sales)) {
  const root = byId('liquidityGrid');
  if (!root) return;
  const inflow = new Map();
  activeSales(filteredSales).forEach((s) => {
    const key = paymentMethodOf(s);
    inflow.set(key, (inflow.get(key) || 0) + n(s.price));
  });
  const expenseMap = expensesByMethod(expenses);
  const methods = [...new Set([...inflow.keys(), ...expenseMap.keys(), 'بنك فلسطين', 'بال باي', 'مال تشات', 'USDT', 'كاش'])];
  const cards = methods.map((method) => {
    const income = inflow.get(method) || 0;
    const out = expenseMap.get(method) || 0;
    const balance = income - out;
    return { method, income, out, balance };
  }).sort((a, b) => b.balance - a.balance);
  const totalBalance = cards.reduce((sum, c) => sum + c.balance, 0);
  root.innerHTML = cards.map((c) => `
    <div class="rounded-2xl bg-slate-50 dark:bg-slate-950/50 border dark:border-slate-800 p-4">
      <div class="metric-label">${escapeHtml(c.method)}</div>
      <div class="text-2xl font-black ${c.balance >= 0 ? 'text-emerald-600' : 'text-red-600'}">${shekel(c.balance)}</div>
      <div class="metric-note">داخل ${shekel(c.income)} • خارج ${shekel(c.out)}</div>
    </div>
  `).join('') + `
    <div class="rounded-2xl bg-slate-900 text-white border border-slate-800 p-4">
      <div class="text-[10px] font-black text-slate-400">إجمالي السيولة التقديرية</div>
      <div class="text-2xl font-black ${totalBalance >= 0 ? 'text-emerald-300' : 'text-red-300'}">${shekel(totalBalance)}</div>
      <div class="text-[11px] mt-2 text-slate-400 font-bold">الإيرادات حسب وسيلة الدفع ناقص المصاريف المسجلة.</div>
    </div>
  `;
}

function customerKey(s) {
  return s.phoneNormalized || normalizePhone(s.clientPhone || s.linkedSaleClientPhone || '') || String(s.clientName || s.linkedSaleClientName || '').trim().toLowerCase();
}
...
function customerProfiles() {
  ...
  activeSales(liveRecords(sales)).forEach((s) => {
    ...
    const old = map.get(key) || {
      ...
      name: s.clientName || '-',
      phone: s.clientPhone || '',
      phoneNormalized: s.phoneNormalized || normalizePhone(s.clientPhone || ''),
      services: new Set(),
      revenue: 0,
      profit: 0,
      count: 0,
    ...
    old.name = s.clientName || old.name;
    old.phone = s.clientPhone || old.phone;
    old.services.add(s.service || 'غير محدد');
    old.revenue += n(s.price);
    old.profit += saleTrueProfit(s);
    old.count += 1;
    if (s.renewedFromSaleId || String(s.label || '').includes('تجديد')) old.renewals += 1;
    if (String(transactionDate(s)) >= String(old.lastDate || '')) {
      old.lastDate = transactionDate(s);
      old.lastService = s.service || '';
      old.lastSaleId = s.id;
    }
    map.set(key, old);
  });
  return [...map.values()].map((c) => ({ ...c, services: [...c.services] }));
}

function renderCustomers() {
  const root = byId('customersGrid');
  if (!root) return;
  const q = String(byId('customerSearch')?.value || '').trim().toLowerCase();
  const list = customerProfiles()
    .filter((c) => !q || `${c.name} ${c.phone} ${c.phoneNormalized} ${c.services.join(' ')}`.toLowerCase().includes(q))
    .sort((a, b) => b.profit - a.profit);
  if (!list.length) {
    root.innerHTML = '<div class="panel text-center text-slate-500 font-bold py-10 lg:col-span-2 xl:col-span-3">لا توجد ملفات عملاء مطابقة.</div>';
    return;
  }
    ...
    <div class="panel space-y-4">
      <div class="flex items-start justify-between gap-3">
        <div>
          ...
          <div class="text-xs text-slate-500 font-bold" dir="ltr">${escapeHtml(c.phone)}</div>
        </div>
        <span class="badge badge-sky">${c.count} عملية</span>
      </div>
      <div class="grid grid-cols-2 gap-3 text-center">
        <div class="rounded-2xl bg-slate-50 dark:bg-slate-950/50 p-3"><div class="metric-label">الإيراد</div><div class="font-black text-sky-600">${shekel(c.revenue)}</div></div>
        <div class="rounded-2xl bg-slate-50 dark:bg-slate-950/50 p-3"><div class="metric-label">الربح</div><div class="font-black ${c.profit >= 0 ? 'text-emerald-600' : 'text-red-600'}">${shekel(c.profit)}</div></div>
      </div>
      <div class="text-xs text-slate-500 font-bold leading-6">آخر خدمة: ${escapeHtml(c.lastService || '-')} • ${escapeHtml(c.lastDate || '-')}<br>التجديدات: ${c.renewals} • الخدمات: ${escapeHtml(c.services.slice(0, 4).join('، '))}</div>
      <div class="flex flex-wrap gap-2">
        ${c.lastSaleId ? `<button onclick="openRenewalFromSale('${c.lastSaleId}')" class="btn-primary px-3 py-2 text-xs">تجديد</button>` : ''}
        <button onclick="window.open('https://wa.me/${escapeHtml(c.phoneNormalized)}', '_blank')" class="btn-light px-3 py-2 text-xs">واتساب</button>
      </div>
    </div>
  `).join('');
}

function showDuplicateCustomerHint() {
  const hint = byId('duplicateCustomerHint');
  if (!hint) return;
  const phone = normalizePhone(byId('inClientPhone')?.value || '');
  if (!phone) {
    hint.classList.add('hidden');
    return;
  }
  const matches = activeSales(sales).filter((s) => (s.phoneNormalized || normalizePhone(s.clientPhone || '')) === phone);
  if (!matches.length) {
    hint.classList.add('hidden');
    return;
  }
  const latest = matches.sort((a, b) => String(transactionDate(b)).localeCompare(String(transactionDate(a))))[0];
  hint.innerHTML = `هذا العميل موجود سابقاً: <b>${escapeHtml(latest.clientName || '-')}</b> • آخر خدمة ${escapeHtml(latest.service || '-')} • ${escapeHtml(transactionDate(latest))}. <button type="button" class="underline" onclick="fillCustomerFromSale('${latest.id}')">تعبئة الاسم</button>`;
  hint.classList.remove('hidden');
}

function fillCustomerFromSale(id) {
  const sale = sales.find((s) => s.id === id);
  if (!sale) return;
  if (byId('inClientName')) byId('inClientName').value = sale.clientName || '';
  if (byId('inSupplier')) byId('inSupplier').value = paymentMethodOf(sale);
  if (byId('inPurchaseSource')) byId('inPurchaseSource').value = purchaseSourceOf(sale);
  toast('تمت تعبئة بيانات العميل من آخر عملية.', 'success');
}

function openRenewalFromSale(id) {
  const sale = sales.find((x) => x.id === id);
  if (!sale || isCompensationRecord(sale)) return toast('لا يمكن تجديد هذه العملية.', 'warning');
  openModalForAdd();
  byId('modalTitle').innerText = 'تجديد اشتراك';
  byId('inService').value = sale.service || '';
  byId('inClientName').value = sale.clientName || '';
  byId('inClientPhone').value = sale.clientPhone || '';
  byId('inSupplier').value = paymentMethodOf(sale);
  if (byId('inPurchaseSource')) byId('inPurchaseSource').value = purchaseSourceOf(sale);
  byId('inSourceLink').value = sale.sourceLink || '';
  byId('inLabel').value = 'تجديد';
  const start = sale.endDate && !Number.isNaN(new Date(sale.endDate).getTime()) ? new Date(sale.endDate) : new Date();
  start.setDate(start.getDate() + 1);
  const end = new Date(start);
  const oldStart = new Date(sale.startDate || start);
  const oldEnd = new Date(sale.endDate || end);
  const duration = Math.max(1, Math.round((oldEnd - oldStart) / 86400000));
  end.setDate(end.getDate() + duration);
  byId('inStart').value = start.toISOString().slice(0, 10);
  byId('inEnd').value = end.toISOString().slice(0, 10);
  byId('inPurchaseCurrency').value = sale.purchaseCurrency || 'USD';
  byId('inCostUSD').value = sale.purchaseCost ?? sale.costUSD ?? '';
  byId('inExchange').value = sale.exchangeRate ?? '3.70';
  byId('inPrice').value = sale.price ?? '';
  byId('inAccountStatus').value = 'تم التجديد';
  if (byId('inFollowupStatus')) byId('inFollowupStatus').value = 'تم التجديد';
  byId('inNotes').value = `تجديد من العملية: ${sale.id}`;
  byId('editSaleId').dataset.renewedFromSaleId = sale.id;
  calculateFinancialPreview();
}

function quickFollowupUpdate(id) {
  const sale = sales.find((x) => x.id === id);
  if (!sale) return;
  const options = ['تم التواصل', 'ينتظر الرد', 'تم التجديد', 'لا يريد التجديد', 'رقم غير صالح', 'جديد'];
  const current = sale.followupStatus || 'جديد';
  const next = prompt('حالة المتابعة الجديدة:\n' + options.join(' / '), current);
  if (!next) return;
  const clean = next.trim();
  const note = prompt('ملاحظة متابعة اختيارية:', sale.followupNote || '') || '';
  db.collection('sales').doc(id).update({
    followupStatus: clean,
    followupNote: note.trim(),
    followupUpdatedAt: firebase.firestore.FieldValue.serverTimestamp(),
    followupUpdatedBy: currentUser?.email || '',
    updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
    updatedBy: currentUser?.email || ''
  }).then(() => writeAudit('followup_updated', { saleId: id, service: sale.service, clientName: sale.clientName, before: sale.followupStatus || '', after: clean, note: note.trim() }))
    .then(() => toast('تم تحديث المتابعة.', 'success'))
    .catch((err) => toast(translateFirebaseError(err), 'error'));
}

function renderMobileSalesCards(data) {
  const root = byId('salesMobileCards');
  if (!root) return;
  const rows = data || currentSalesFilter();
  if (!rows.length) {
    root.innerHTML = '<div class="text-center text-slate-500 font-bold py-6">لا توجد عمليات مطابقة.</div>';
    return;
  }
  root.innerHTML = rows.slice(0, 80).map((s) => {
    const isComp = isCompensationRecord(s);
    const st = statusOfSale(s);
    const profit = isComp ? -compensationAmount(s) : saleTrueProfit(s);
    return `
      <div class="sale-mobile-card">
        <div class="flex items-start justify-between gap-3">
          <div><div class="font-black text-base">${escapeHtml(saleTitle(s))}</div><div class="text-xs text-slate-500 font-bold">${escapeHtml(s.clientName || s.linkedSaleClientName || '-')} • ${escapeHtml(transactionDate(s))}</div></div>
          <span class="badge ${st.cls}">${st.text}</span>
        </div>
        <div class="grid grid-cols-3 gap-2 text-center text-xs mt-3">
          <div class="rounded-xl bg-slate-50 dark:bg-slate-950/50 p-2"><div class="metric-label">إيراد</div><b>${shekel(n(s.price))}</b></div>
          <div class="rounded-xl bg-slate-50 dark:bg-slate-950/50 p-2"><div class="metric-label">تكلفة</div><b>${shekel(saleTotalCost(s))}</b></div>
          <div class="rounded-xl bg-slate-50 dark:bg-slate-950/50 p-2"><div class="metric-label">ربح</div><b class="${profit >= 0 ? 'text-emerald-600' : 'text-red-600'}">${shekel(Math.abs(profit))}</b></div>
        </div>
        <div class="flex flex-wrap gap-2 mt-3">
          ${!isComp ? `<button onclick="openModalForEdit('${s.id}')" class="icon-btn text-xs">✏️</button><button onclick="openRenewalFromSale('${s.id}')" class="icon-btn text-xs">🔁</button><button onclick="quickFollowupUpdate('${s.id}')" class="icon-btn text-xs">📌</button><button onclick="window.open('${whatsappUrl(s)}', '_blank')" class="icon-btn text-xs">💬</button>` : ''}
          <button onclick="deleteSale('${s.id}')" class="icon-btn text-xs">🗑️</button>
        </div>
      </div>
    `;
  }).join('');
...
function calculateExpensePreview() {
  const amountUSD = n(byId('expenseAmountUSD')?.value);
  const exchangeRate = n(byId('expenseExchangeRate')?.value);
  const amountILS = amountUSD * exchangeRate;
  const target = byId('expenseAmount');
  if (target) target.value = amountILS > 0 ? amountILS.toFixed(3) : '';
  return { amountUSD, exchangeRate, amountILS };
}

function resetExpenseForm() {
  const form = byId('expenseForm');
  if (form) form.reset();
  if (byId('expenseEditId')) byId('expenseEditId').value = '';
  if (byId('expenseDate')) byId('expenseDate').value = todayISO();
  if (byId('expenseExchangeRate')) byId('expenseExchangeRate').value = byId('expenseExchangeRate').value || '3.70';
  if (byId('expenseExchangeRate')) byId('expenseExchangeRate').value = '3.70';
  if (byId('expenseAmount')) byId('expenseAmount').value = '';
  if (byId('saveExpenseBtn')) byId('saveExpenseBtn').innerText = 'حفظ المصروف';
}
...
function setupExpenseForm() {
  ...
  form.addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!currentUser) return toast('سجّل الدخول أولاً.', 'warning');
    const id = byId('expenseEditId').value;
    const financial = calculateExpensePreview();
    const data = {
      date: byId('expenseDate').value || todayISO(),
      title: byId('expenseTitle').value.trim(),
      category: byId('expenseCategory').value,
      paymentMethod: byId('expensePaymentMethod').value,
      currency: 'USD',
      amountUSD: financial.amountUSD,
      exchangeRate: financial.exchangeRate,
      amount: financial.amountILS,
      amountILS: financial.amountILS,
      notes: byId('expenseNotes').value.trim(),
      updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      updatedBy: currentUser.email,
      updatedByName: myProfile.permanentName
    };
    if (!data.title || data.amountUSD <= 0 || data.exchangeRate <= 0 || data.amount <= 0) return toast('أدخل بند المصروف والمبلغ بالدولار وسعر الصرف بشكل صحيح.', 'warning');
    const btn = byId('saveExpenseBtn');
    setLoading(btn, true);
    try {
      if (id) {
        const before = expenses.find((x) => x.id === id) || {};
        await db.collection('expenses').doc(id).update(data);
        await writeAudit('expense_updated', { expenseId: id, title: data.title, amountUSD: data.amountUSD, exchangeRate: data.exchangeRate, amount: data.amount, before: plainCopy(before), after: plainCopy(data) });
        toast('تم تعديل المصروف.', 'success');
      } else {
        data.createdAt = firebase.firestore.FieldValue.serverTimestamp();
        data.createdBy = currentUser.email;
        data.createdByName = myProfile.permanentName;
        const ref = await db.collection('expenses').add(data);
        await writeAudit('expense_created', { expenseId: ref.id, title: data.title, amountUSD: data.amountUSD, exchangeRate: data.exchangeRate, amount: data.amount, category: data.category });
        toast('تم حفظ المصروف.', 'success');
      }
      resetExpenseForm();
      setLoading(btn, false, 'حفظ المصروف');
    } catch (err) {
      toast(translateFirebaseError(err), 'error');
      setLoading(btn, false, 'حفظ المصروف');
    }
...
function renderExpenses() {
  setupExpenseForm();
  const root = byId('expensesList');
  if (!root) return;
  const q = String(byId('expenseSearch')?.value || '').trim().toLowerCase();
  const items = expenses.filter((x) => !x.isDeleted).filter((x) => !q || `${x.title} ${x.category} ${x.paymentMethod} ${x.notes} ${x.amountUSD || ''} ${x.exchangeRate || ''} ${x.amount || ''}`.toLowerCase().includes(q));
  const total = expensesTotal(items);
  if (byId('expensesSummary')) byId('expensesSummary').innerText = `${items.length} مصروف • الإجمالي ${shekel(total)}`;
  if (!items.length) {
    root.innerHTML = '<div class="p-8 text-center text-slate-500 font-bold">لا توجد مصاريف مسجلة.</div>';
    return;
  }
  root.innerHTML = items.map((x) => `
    <div class="p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-900">
      <div><div class="font-black">${escapeHtml(x.title)}</div><div class="text-xs text-slate-500 font-bold mt-1">${escapeHtml(x.date || '-')} • ${escapeHtml(x.category || '-')} • ${escapeHtml(paymentMethodOf(x))}</div><div class="text-xs text-slate-400 font-bold mt-1">${n(x.amountUSD) ? '$' + n(x.amountUSD).toFixed(3) + ' × ' + n(x.exchangeRate || 1).toFixed(3) + ' = ' : ''}${shekel(x.amount)}</div>${x.notes ? `<div class="text-xs text-slate-400 mt-2">${escapeHtml(x.notes)}</div>` : ''}</div>
      <div class="flex items-center gap-2"><div class="font-black text-red-600">${shekel(x.amount)}</div><button onclick="editExpense('${x.id}')" class="icon-btn text-xs">✏️</button><button onclick="deleteExpense('${x.id}')" class="icon-btn text-xs">🗑️</button></div>
    </div>
  `).join('');
}
...
function editExpense(id) {
  ...
  byId('expenseEditId').value = x.id;
  byId('expenseDate').value = x.date || todayISO();
  byId('expenseTitle').value = x.title || '';
  byId('expenseCategory').value = x.category || 'أخرى';
  byId('expensePaymentMethod').value = x.paymentMethod || 'أخرى';
  const rate = n(x.exchangeRate) || 3.70;
  byId('expenseExchangeRate').value = rate;
  byId('expenseAmountUSD').value = n(x.amountUSD) || (n(x.amount) > 0 ? (n(x.amount) / rate).toFixed(3) : '');
  calculateExpensePreview();
  byId('expenseNotes').value = x.notes || '';
  byId('saveExpenseBtn').innerText = 'تحديث المصروف';
}
...
async function deleteExpense(id) {
  ...
  const x = expenses.find((e) => e.id === id) || {};
  try {
    await db.collection('expenses').doc(id).update({ isDeleted: true, deletedAt: firebase.firestore.FieldValue.serverTimestamp(), deletedBy: currentUser?.email || '' });
    await writeAudit('expense_deleted', { expenseId: id, title: x.title || '', amount: x.amount || 0 });
...
async function validateSelectedBackupFile() {
  const input = byId('backupFileInput');
  const file = input?.files?.[0];
  if (!file) return toast('اختر ملف النسخة أولاً.', 'warning');
  try {
    const payload = await readBackupFile(file);
    const keys = validateBackupPayload(payload);
    const total = countBackupDocuments(Object.fromEntries(keys.map((key) => [key, payload.collections[key]])));
    if (byId('backupExportInfo')) byId('backupExportInfo').innerText = `فحص النسخة: الملف صالح ويحتوي على ${total} سجل ضمن: ${keys.join(', ')}. تاريخ النسخة: ${payload.generatedAt || 'غير محدد'}.`;
    toast('ملف النسخة صالح للاستعادة.', 'success');
  } catch (err) {
    toast(err.message || 'ملف النسخة غير صالح.', 'error');
  }
}

function setupBackupConfirmVisibility() {
  const checkbox = byId('backupReplaceMode');
  const input = byId('backupConfirmInput');
  if (!checkbox || !input || checkbox.dataset.enhanced === '1') return;
  checkbox.dataset.enhanced = '1';
  const update = () => input.classList.toggle('hidden', !checkbox.checked);
  checkbox.addEventListener('change', update);
  update();
}

function setupPwaAndOffline() {
  if (enhancedPwaReady) return;
  enhancedPwaReady = true;
  const update = () => {
    byId('offlineBanner')?.classList.toggle('hidden', navigator.onLine);
  };
  window.addEventListener('online', () => { update(); toast('عاد الاتصال بالإنترنت.', 'success'); });
  window.addEventListener('offline', update);
  update();
  try {
    if (firebase?.firestore && firebase.firestore().enablePersistence) {
      firebase.firestore().enablePersistence({ synchronizeTabs: true }).catch(() => null);
    }
  } catch (err) {
...
const originalSaleFormSubmitBridge = () => {
  const form = byId('saleForm');
  if (!form || form.dataset.renewalBridge === '1') return;
  form.dataset.renewalBridge = '1';
  form.addEventListener('submit', () => {
    const renewedFrom = byId('editSaleId')?.dataset?.renewedFromSaleId || '';
    if (renewedFrom && !byId('editSaleId').value) {
      setTimeout(async () => {
        try {
          const recent = activeSales(sales).find((s) => s.renewedFromSaleId === renewedFrom);
          await writeAudit('sale_renewed', { renewedFromSaleId: renewedFrom, newSaleId: recent?.id || '', service: byId('inService')?.value || '', clientName: byId('inClientName')?.value || '' });
        } catch (_) {}
        if (byId('editSaleId')) delete byId('editSaleId').dataset.renewedFromSaleId;
      }, 1200);
    }
  }, true);
};

const _oldCalculateFinancialPreview = calculateFinancialPreview;
calculateFinancialPreview = function () {
  return _oldCalculateFinancialPreview();
};

const _oldRenderSalesTable = renderSalesTable;
renderSalesTable = function (data) {
  _oldRenderSalesTable(data);
  renderMobileSalesCards(data);
};

const _oldUpdateAnalytics = updateAnalytics;
updateAnalytics = function () {
  _oldUpdateAnalytics();
  const grid = byId('anaGrid');
  if (grid && !byId('anaExpenseCard')) {
    const total = expensesTotal();
    grid.insertAdjacentHTML('beforeend', `<div id="anaExpenseCard" class="metric-card text-center"><div class="metric-label">المصاريف العامة</div><div class="metric-value text-red-600">${shekel(total)}</div><div class="metric-note">تخصم من السيولة وليس من كل مبيعة</div></div>`);
  } else if (byId('anaExpenseCard')) {
    byId('anaExpenseCard').querySelector('.metric-value').innerText = shekel(expensesTotal());
  }
};

const _oldOpenModalForAdd = openModalForAdd;
openModalForAdd = function () {
  _oldOpenModalForAdd();
  if (byId('editSaleId')) delete byId('editSaleId').dataset.renewedFromSaleId;
};

// Add renewedFromSaleId to newly saved renewal records through a small Firestore follow-up.
const _oldWriteAudit = writeAudit;
writeAudit = async function (action, payload = {}) {
  return _oldWriteAudit(action, payload);
};

// Patch sale save by adding renewal relation before original submit reads data via existing fields.
byId('saleForm')?.addEventListener('submit', async () => {
  const renewedFrom = byId('editSaleId')?.dataset?.renewedFromSaleId || '';
  if (renewedFrom) {
    window.__pendingRenewedFromSaleId = renewedFrom;
  }
}, true);

// Attach a lightweight post-save marker for renewals by finding the newest matching sale.
byId('saleForm')?.addEventListener('submit', () => {
  const renewedFrom = window.__pendingRenewedFromSaleId;
  if (!renewedFrom) return;
  setTimeout(async () => {
    try {
      const snap = await db.collection('sales')
        .where('addedByEmail', '==', currentUser.email)
        .orderBy('updatedAt', 'desc')
        .limit(5)
        .get();
      const target = snap.docs.map((doc) => ({ id: doc.id, ...doc.data() })).find((s) => !s.renewedFromSaleId && s.service === byId('inService')?.value && normalizePhone(s.clientPhone || '') === normalizePhone(byId('inClientPhone')?.value || ''));
      if (target) await db.collection('sales').doc(target.id).set({ renewedFromSaleId: renewedFrom, renewalCreatedAt: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
    } catch (err) {
      console.warn('Renewal marker failed', err);
    } finally {
      window.__pendingRenewedFromSaleId = '';
    }
  }, 1500);
}, false);
...
window.addEventListener('load', () => {
  setupExpenseForm();
  setupBackupConfirmVisibility();
  setupPwaAndOffline();
  originalSaleFormSubmitBridge();
  renderCustomers();
  renderExpenses();
  renderMobileSalesCards();
});
