...
function showSection(id, event) {
  if ((id === 'userMgmt' || id === 'backup') && myProfile.role !== 'SuperAdmin') {
...
async function exportFullBackup() {
  ...
  try {
    ...
    byId('backupExportInfo').innerText = 'آخر تصدير: ' + total + ' سجل. ' + (Object.keys(errors).length ? 'بعض المجموعات لم تُقرأ بسبب الصلاحيات.' : 'تم تصدير كل المجموعات المتاحة.');
...
function validateBackupPayload(payload) {
  ...
  keys.forEach((key) => {
    ...
    payload.collections[key].forEach((item) => {
      if (!item || typeof item.id !== 'string' || !item.id || typeof item.data !== 'object') {
        throw new Error('يوجد سجل غير صالح داخل المجموعة: ' + key);
      }
    });
  });
  return keys;
...
async function restoreFullBackup() {
  ...
  const message = replaceMode
    ? 'سيتم حذف البيانات الحالية في المجموعات الموجودة بالنسخة ثم استعادتها من الملف. هل أنت متأكد؟'
    : 'سيتم دمج بيانات النسخة مع البيانات الحالية بدون حذف الموجود. هل تريد المتابعة؟';
  ...
  try {
    ...
    keys.forEach((key) => {
      payload.collections[key].forEach((item) => {
        operations.push({ type: 'set', ref: db.collection(key).doc(item.id), data: restoreBackupValue(item.data) });
      });
    });
