...
function setupExpenseForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const data = {
      ...
      amount: n(byId('expenseAmount').value),
      notes: byId('expenseNotes').value.trim(),
      updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      updatedBy: currentUser.email,
      updatedByName: myProfile.permanentName
    };
    if (!data.title || data.amount <= 0) return toast('أدخل بند المصروف والمبلغ بشكل صحيح.', 'warning');
    ...
    try {
      if (id) {
        ...
        await writeAudit('expense_updated', { expenseId: id, title: data.title, amount: data.amount, before: plainCopy(before), after: plainCopy(data) });
        ...
        await writeAudit('expense_created', { expenseId: ref.id, title: data.title, amount: data.amount, category: data.category });
        toast('تم حفظ المصروف.', 'success');
      }
      form.reset();
      byId('expenseEditId').value = '';
...
function renderExpenses() {
  ...
  const items = expenses.filter((x) => !x.isDeleted).filter((x) => !q || `${x.title} ${x.category} ${x.paymentMethod} ${x.notes}`.toLowerCase().includes(q));
  ...
  root.innerHTML = items.map((x) => `
    <div class="p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-900">
      <div><div class="font-black">${escapeHtml(x.title)}</div><div class="text-xs text-slate-500 font-bold mt-1">${escapeHtml(x.date || '-')} • ${escapeHtml(x.category || '-')} • ${escapeHtml(paymentMethodOf(x))}</div>${x.notes ? `<div class="text-xs text-slate-400 mt-2">${escapeHtml(x.notes)}</div>` : ''}</div>
...
function editExpense(id) {
  ...
  byId('expenseAmount').value = x.amount || '';
